...
function createFaq() {
  for (const item in faqData) {
    ...
    let faqNav = document.createElement('h3')
    faqNav.className = 'faq_header'
    faqNav.id = 'q' + faqData[item].id
    faqNav.textContent = faqData[item].question
    faqItem.appendChild(faqNav)

    // h3 Element created for Faq answer
